...
flowchart LR
  ...
  subgraph EDGE["Lớp biên"]
    NG["🌐 Nginx<br/>SPA + prerender<br/>proxy /api"]
  end
  subgraph APP["Lớp ứng dụng"]
    API["⚙️ Express API<br/>/api/v1"]
  end
  ...
  U -->|HTTPS| NG -->|/api/v1| API
  API --> DB
  API --> RD
  API --> CL
  API --> PAY
  API --> MAIL
  PAY -.webhook HMAC.-> API
```
...
```
  HTTP request
      ...
      ▼
╭────────────────────────────────────────────╮
│  ① MIDDLEWARES   helmet · cors · csrf · rate-limit  │  ◀─ bảo vệ & lọc
│                  sanitize · auth (JWT) · validate    │     mọi request
╰───────────────────────────┬───────────────╯
                              ▼
╭────────────────────────────────────────────╮
│  ② ROUTES        định tuyến URL → controller          │  routes/
╰───────────────────────────┬───────────────╯
                              ▼
╭────────────────────────────────────────────╮
│  ③ CONTROLLERS   đọc req, gọi service, trả res       │  controllers/
│                  (KHÔNG chứa logic nghiệp vụ)         │
╰───────────────────────────┬───────────────╯
                              ▼
╭────────────────────────────────────────────╮
│  ④ SERVICES      ⭐ TRÁI TIM nghiệp vụ:              │  services/
│                  pricing-engine, order, promotion   │
│                  giao dịch (transaction), tính giá  │
╰───────────────────────────┬───────────────╯
                              ▼
╭────────────────────────────────────────────╮
│  ⑤ MODELS        Mongoose schema (~22) ↔ MongoDB     │  models/
╰────────────────────────────────────────────╯
```
...
sequenceDiagram
  ...
  C->>M: POST /orders (+ JWT, CSRF)
  M->>M: helmet, cors, rate-limit, sanitize
  M->>M: xác thực JWT, validate body (Zod)
  M->>R: hợp lệ → chuyển tiếp
  R->>Ct: orders.create()
  Ct->>S: createOrder(userId, items, voucher)
  S->>P: resolvePrice(variant) cho từng dòng
  P-->>S: giá cuối (đã tính Flash/Discount)
  S->>DB: withTransaction() — trừ tồn, trừ suất flash, snapshot giá
  DB-->>S: commit OK
  S-->>Ct: order đã tạo
  Ct-->>C: 201 Created + mã đơn
```
...
```mermaid
flowchart TD
  A(["Giá của 1 biến thể"]) --> B{"Đang có<br/>Flash Sale?"}
  B -- Có --> FS["Dùng salePrice"]
  B -- Không --> C{"Có Discount<br/>hợp lệ?"}
  C -- Có --> D["Áp Discount<br/>priority cao nhất"]
  C -- Không --> E["Giá niêm yết<br/>(basePrice)"]
  FS --> Z(["Giá cuối"])
  D --> Z
  E --> Z
  Z -.snapshot vào đơn.-> ORDER["🧾 OrderItem"]
```
